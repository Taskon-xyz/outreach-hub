...
Web3 Outreach Hub — Python 桌面应用（CustomTkinter GUI），用于 Web3 项目的自动化触达：从多种数据源导入项目 → 爬取官网提取 Telegram/X 链接 → 解析群管理员/离群用户 → Playwright 自动发送 DM（TG 走 web.telegram.org，X 走 CDP Chrome）。数据存储在本地 SQLite。

**运行环境**：Windows / macOS 桌面（发送依赖 Playwright 启动的真实浏览器，需桌面环境显示），Python 3.11+。
...
# 启动桌面 GUI（macOS：CDP Chrome + GUI 一体脚本）
./scripts/start_chrome_cdp.sh
# Windows：双击 scripts\start_chrome_cdp.bat（或 install 生成的 start.bat）
...
tg_handles 表（管理员） + tg_left_users 表（离群用户）
    ↓ SenderWorker（Playwright 浏览器自动化）
send_log 表
```
...
| `tg_sender_web_worker` | TG handles | send_log | Playwright（web.telegram.org，两平台统一） |
| `x_sender_pw_worker` | X handles | send_log | Playwright（CDP 连真实 Chrome，两平台统一） |
...
- 桌面应用（Windows/macOS），发送走 Playwright 启动的真实浏览器；macOS 上可改代码，浏览器自动化行为建议在目标平台验证
...
- （旧坐标/OCR 发送方案已移除）发送统一走 Playwright；历史坐标校准数据仍在 `settings` 表，但不再使用
